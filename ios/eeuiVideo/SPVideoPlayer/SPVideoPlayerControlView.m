...
//        [self addSubview:self.placeholderView];
        ...
        [_pauseBtn setImage:SPPlayerImage(@"player_full_pause_h_iphone_30x30_") forState:UIControlStateHighlighted];
        [_pauseBtn setImage:SPPlayerImage(@"player_full_pause_iphone_30x30_") forState:UIControlStateNormal];
        [_pauseBtn setImage:SPPlayerImage(@"player_full_play_h_iphone_30x30_") forState:UIControlStateHighlighted];
        [_pauseBtn setImage:SPPlayerImage(@"player_full_play_iphone_30x30_") forState:UIControlStateSelected];
    }
    return self;
}
...
- (void)showControlView {
    ...
    SPPlayerShared.isStatusBarHidden = NO;
    //
    if ((_topBackButtonType == 1 && !_fullScreenMode) || _topBackButtonType == 2) {
        self.topView.backButtonHidden = YES;
        self.topView.titleHidden = YES;
    }else{
        self.topView.backButtonHidden = NO;
        self.topView.titleHidden = NO;
    }
...
-(void)setImg:(NSString*)url mInstance:(WXSDKInstance*)mInstance {
    ...
    [SDWebImageDownloader.sharedDownloader downloadImageWithURL:ul options:SDWebImageDownloaderLowPriority progress:nil completed:^(UIImage * _Nullable image, NSData * _Nullable data, NSError * _Nullable error, BOOL finished) {
        if (image) {
            self->_placeholderView.placeholderImageView.image = image;
        }
    }];
}
...
- (void)layoutSubviews {
    ...
    self.lockBtn.frame = CGRectMake(36, 0, 40, 40);
...
- (void)layoutSubviews {
    ...
    if (self.backButtonHidden) {
        CGFloat videoTitleLabelX = 12;
        CGFloat videoTitleLabelY = backButtonY - 10;
    ...
    self.videoTitleLabel.hidden = _titleHidden;
